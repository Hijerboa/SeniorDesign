...
# %%
def load_model(model_file: str, level):
    location = os.path.realpath(os.path.join(os.getcwd(), os.path.dirname(__file__)))
    if level >= 0:
        model_file = f'{model_file}_{level}'
...
# %%
def TRAIN(input_config: Config, output_config: Config, level):
    """
    ...
    if input_config == None:
        ...
        model = load_model(input_config.model_file, level-1)
    ...
    model.save_pretrained(f'{path}_{level}', save_model=True)
...
# %%
# TRAIN(shuffled_train_config, shuffled_train_config, 69)

# %% [markdown]
# ## EVALUATION
model = load_model(shuffled_train_config.model_file, 4)
